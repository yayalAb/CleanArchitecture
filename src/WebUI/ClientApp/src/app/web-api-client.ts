...
export interface IProductClient {
    ...
    createProducts(command: CreateProductItemCommand): Observable<Product>;
    update(productId: number | undefined, id: string, command: UpdateProductCommand): Observable<FileResponse>;
...
export class ProductClient implements IProductClient {
    ...
    createProducts(command: CreateProductItemCommand): Observable<Product> {
        ...
        return this.http.request("post", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processCreateProducts(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processCreateProducts(response_ as any);
                } catch (e) {
    ...
    protected processCreateProducts(response: HttpResponseBase): Observable<Product> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
